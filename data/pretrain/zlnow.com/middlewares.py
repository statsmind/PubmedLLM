...
class DownloaderMiddleware:
    ...
    # passed objects.
    CACHE_DIR = r"Z:\datasets\www.zlnow.com"
    ...
    def process_request(self, request, spider):
        ...
        local_cache_file = self.get_local_cache_file(request)
        if os.path.exists(local_cache_file):
            with open(local_cache_file, 'r', encoding='utf8') as fp:
                print(request.url, "cached")

                response = HtmlResponse(url=request.url, body=fp.read(), encoding='utf8', request=request)
                response.meta['cached'] = True

                request.meta["download_timeout"] = 0
                return response
        ...
        if response.status == 200:
            ...
            if not os.path.exists(local_cache_file):
                ...
                if len(html_content) > 0:
                    with open(local_cache_file, 'w', encoding='utf-8') as fp:
